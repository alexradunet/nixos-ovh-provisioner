...
function createDeployHarness() {
	...
	const fakeNixPath = path.join(rootDir, "fake-nix.sh");
	const passwordFilePath = path.join(rootDir, "generated-root-password.txt");
...
`,
	...
	fs.writeFileSync(
		...
		`#!/usr/bin/env bash
set -euo pipefail
printf '$6$rounds=500000$testsalt$autogeneratedhash\\n'
`,
	);
	fs.chmodSync(fakeNixPath, 0o755);
...
async function runDeploy(
	...
	const result = await run("bash", [deployScriptPath, ...args], undefined, overrides?.cwd ?? repoRoot, {
		...
		PLAIN_HOST_DEPLOY_PASSWORD_FILE: harness.passwordFilePath,
		TMPDIR: harness.rootDir,
		NIX_BIN: harness.fakeNixPath,
		...overrides?.env,
	});
	...
	return {
		...
		readGeneratedPasswordFile() {
			return fs.readFileSync(harness.passwordFilePath, "utf8");
		},
	};
}
...
describe("plain-host-deploy.sh", () => {
	...
	it("saves an auto-generated root password to a local root-only file", async () => {
		const result = await runDeploy(["--target-host", "root@198.51.100.10", "--disk", "/dev/sda"]);

		try {
			expect(result.exitCode).toBe(0);
			expect(result.stderr).toContain(`Saved generated root password to ${result.harness.passwordFilePath}`);
			expect(result.stderr).toContain("Generated root password for KVM console access:");
			expect(fs.statSync(result.harness.passwordFilePath).mode & 0o777).toBe(0o600);
			expect(result.readGeneratedPasswordFile()).toContain(
				"plain-host-deploy generated OVH KVM root password",
			);
			expect(result.readGeneratedPasswordFile()).toContain("password=");
			expect(result.readGeneratedFlake()).toContain("users.users.root.hashedPassword = lib.mkForce");
			expect(result.readGeneratedFlake()).toContain("$6$rounds=500000$testsalt$autogeneratedhash");
		} finally {
			result.harness.cleanup();
		}
	});
